...
package team.cakeslayers.xiguajerry.legacy.myrpg.role

import team.cakeslayers.xiguajerry.legacy.myrpg.item.Hitable

abstract class AbstractMonster(val name: String, val description: String, override var healthPoints: Int) : Fightable {
    override fun attack(opponent: Fightable, weapon: Hitable) {
        opponent.healthPoints -= weapon.damage
        println("$name use his ${weapon.name} took the opponent ${weapon.damage} HP!")
    }
}